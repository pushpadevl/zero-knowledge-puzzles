pragma circom  2.1.8;

/* Purpose: Check whether the ky expansion worked correctly

*/
function byteSub(A){
    var sbox[256] = [
        ];

    
    for(var i=0;i<4;i++){
        A[i] = sbox[A[i]];    
    }
    return A;
}

template keySch(keySize){
    
    var noOfCols = keySize \ 32 ; //for 128, it's 4, for 192 - 6, 256 - 8
    var noOfSubkeys = noOfCols == 4 ? 10 : (noOfCols ==6 ? 12: 14) ;
    var A[4] = [0x00,0x01,0x0c,0x0d];
    A = byteSub(A);

    signal input in[2];
    signal output check[4];
    for (var i=0; i<4; i++) check[i] <== A[i];

    signal output out[3];

    out[0] <== noOfCols; // for check
    out[1] <== noOfSubkeys; // for check
    out[2] <-- in[0] \ in[1];
    out[2]*in[1] === in[0];
}
component main = keySch(256);

/* DNW
Non quadratic constraint

    out[0] <== a;
    out[1] <== b;
    out[2] <== in[0] \ in[1];

Works

    out[0] <== a;
    out[1] <== b;
    out[2] <-- in[0] \ in[1];
    out[2]*in[1] === in[0];
*/