...
void IsSorted_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void IsSorted_3_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 788;}

uint get_number_of_components() {return 10;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 761;}
...
ctx->componentMemory[coffset].templateName = "IsSorted";
ctx->componentMemory[coffset].signalStart = soffset;
...
while(Fr_isTrue(&expaux[0])){
Fr_sub(&expaux[0],&lvar[1],&circuitConstants[2]); // line circom 15
{
...
Fr_sub(&expaux[0],&lvar[1],&circuitConstants[2]); // line circom 16
{
uint cmp_index_ref = ((1 * Fr_toInt(&expaux[0])) + 0);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 2];
...
Fr_sub(&expaux[1],&lvar[1],&circuitConstants[2]); // line circom 17
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&expaux[1])) + 0)]].signalStart + 0],&circuitConstants[2]); // line circom 17
...
assert(Fr_isTrue(&expaux[0]));
Fr_sub(&expaux[0],&lvar[1],&circuitConstants[2]); // line circom 18
{
...
IsSorted_3_create(1,0,ctx,"main",0);
IsSorted_3_run(0,ctx);
}
